...
def cmd_add_remote(name:str,url:str)->_A:
...
@click.argument(_E,required=_C)
def cmd_delete_remote(name:str)->_A:
...
def cmd_remotes(format_:str)->_A:
...
@publish_invocation
def cmd_pod_delete(name:str,remote:str|_A=_A)->_A:
...
@publish_invocation
def cmd_pod_save(name:str|_A=_A,remote:str|_A=_A,services:str|_A=_A,message:str|_A=_A,visibility:str|_A=_A,secret:str|_A=_A,format_:str|_A=_A)->_A:
	...
	I:list|_A=[A.strip()for A in A.split(',')]if A else _A
	try:D:dict=G.save(pod_name=B,attributes=CloudPodRemoteAttributes(is_public=_B,description=message,services=I),remote=H,secret=secret)
...
@publish_invocation
def cmd_pod_load(name:str|_A=_A,remote:str|_A=_A,strategy:str|_A=_A,yes:bool=_B,secret:str|_A=_A,dry_run:bool=_B)->_A:
...
def get_pod_name_and_version(pod_name:str)->tuple[str,int|_A]:
	A=pod_name
...
@publish_invocation
def cmd_pod_list_pods(remote:str|_A=_A,public:bool=_B,mine:bool=_B,format_:str=_A)->_A:
	...
	F=RemoteConfigParams(remote_name=B)if B else _A;A:list[dict]=D.list(remote=F,creator='me'if mine else _A)
...
@publish_invocation
def cmd_pod_versions(name:str,format_:str)->_A:
	C=CloudPodsClient();_assert_host_reachable()
	try:A:list[dict]=C.get_versions(pod_name=name);A=[A for A in A if not A.get('deleted')]
