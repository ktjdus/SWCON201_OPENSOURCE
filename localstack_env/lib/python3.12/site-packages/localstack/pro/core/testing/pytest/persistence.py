...
class PersistenceSnapshotSession(SnapshotSession):
	...
	def _persist_state(A)->_A:0
	def _load_state(A)->dict:return{}
...
class PersistenceValidator:
	...
	def __init__(A,test:Item,function:Callable,snapshot:PersistenceSnapshotSession):A.test=test;A.function=function;A.snapshot=snapshot
class PersistenceValidations:
	test:Item;validators:list[PersistenceValidator]
	def __init__(A,test:Item,snapshot:PersistenceSnapshotSession):A.test=test;A.validators=[];A.snapshot=snapshot
	def register(A,fn:Callable):A.validators.append(PersistenceValidator(A.test,fn,A.snapshot))
	def run_setup(A):
...
class PersistenceValidationsItem(Item):
	...
	def __init__(A,parent,name,test:Item,validations:PersistenceValidations):super().__init__(parent=parent,name=name);A.own_markers=list(test.own_markers);A.test=test;A.validations=validations;A.funcargs={};A.failed_validator=_A
	...
	def _prunetraceback(B,excinfo:ExceptionInfo[BaseException])->_A:
		G='auto';C=excinfo
...
class LocalstackSession:
	def __init__(A,persistence_mode:LocalstackPersistenceMode,port:int=constants.DEFAULT_PORT_EDGE,tmp_dir:str|_A=_A,bind_host:str|_A=_A):A.port=port;A.tmp_dir=tmp_dir or tempfile.mkdtemp(prefix='localstack-pytest-');A.volume_dir=os.path.join(A.tmp_dir,'localstack-volume');A.persistence_mode=persistence_mode;(A.server):Server|_A=_A;A.env_vars={};A.bind_host=bind_host
	def start(A,timeout:int):
		B=timeout
		if A.server:return
	...
	def restart(A,timeout:int):B=timeout;A.stop(B);A.server=_A;A.start(B)
	def close(A):
...
class PersistenceSession:
	...
	def configure(B,config:Config):A=config;B.config=A;C=A.option.persistence_mode or LocalstackPersistenceMode.snapshot;D=str(A._tmp_path_factory.mktemp('localstack-persistence-tests-',numbered=_B));B.localstack_session=LocalstackSession(C,tmp_dir=D)
	...
	def register_test(B,test:Item)->PersistenceValidationsItem:A=test;E=B._new_snapshot_session(A);C=PersistenceValidations(A,E);D=PersistenceValidationsItem.from_parent(parent=A.parent,name=f"{A.name}[persistence_validations]",test=A,validations=C);B.validations[A.nodeid]=C;B.validations_items[A.nodeid]=D;return D
	def _new_snapshot_session(C,item:Item)->PersistenceSnapshotSession:A=item;B=PersistenceSnapshotSession(base_file_path=os.path.join(os.path.relpath(A.fspath.dirname,ROOT_FOLDER),f"{A.fspath.purebasename}.snapshot.json"),scope_key=A.nodeid,update=_C,verify=_B);B.add_transformer(SNAPSHOT_BASIC_TRANSFORMER,priority=2);return B
	def is_persistence_test(A,item:Item)->bool:
...
class LocalstackDevContainerServer(Server):
	...
	def _get_container_configurators(A)->list[ContainerConfigurator]:
		...
		return B
	def _get_coverage_file_path_on_host(A)->str:return os.path.join(A.volume_dir,COVERAGE_FILE_NAME)
	def do_run(A):
...
class PersistenceTestPlugin:
	@pytest.hookimpl()
	def pytest_addoption(self,parser:Parser,pluginmanager:PytestPluginManager):A=parser;A.addoption('--persistence-splits',dest=_G,type=int,help='The number of groups to split the tests into');A.addoption('--persistence-group',dest=_H,type=int,help='The group of tests that should be executed (first one is 1)');A.addoption('--persistence-mode',type=LocalstackPersistenceMode,choices=list(LocalstackPersistenceMode))
	@pytest.hookimpl(tryfirst=_B)
	def pytest_cmdline_main(self,config:'Config')->int|ExitCode|_A:
	...
	def pytest_configure(self,config:Config):persistence_session.configure(config)
	@pytest.hookimpl(trylast=_B)
	def pytest_sessionstart(self,session:Session):persistence_session.start_localstack()
	@pytest.hookimpl()
	...
	def pytest_collection_modifyitems(self,session:Session,config:Config,items:list[Item]):
		...
		if E and D:H:list[str]=[A.name for A in split_list(lst=B,n=E)[D-1]];B[:]=[A for A in B if A.name in H]
		for A in B:
	...
	def fixture_persistence_validations(self,request:FixtureRequest)->PersistenceValidations:
		A=request
...
def split_list(lst:list,n:int=0)->list[list]:
	A=lst
	if n==0:return[A]
