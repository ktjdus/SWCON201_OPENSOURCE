...
class LicenseV1Client(LicenseV1ClientBase):
	...
	def activate_license_online(B,credentials:Credentials,license:LicenseV1)->LicenseV1:
		...
		C=F.post(f"{constants.API_ENDPOINT}/license/activate",json.dumps({'license_id':license.id,_H:{'type':D,'token':E},_O:B._get_product_data(),_P:B._get_machine_data()}),verify=not config.is_env_true(_Q),proxies=H)
	...
	def _request_license(B,credentials:AuthToken|ApiKeyCredentials)->LicenseV1:
		...
		C=F.post(f"{constants.API_ENDPOINT}/license/request",json.dumps({_H:{'type':D,'token':E},_O:B._get_product_data(),_P:B._get_machine_data()}),verify=not config.is_env_true(_Q),proxies=H)
